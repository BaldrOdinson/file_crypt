...
def get_destination_file():
    ...
    destination_path.place(x=165, y=140)
...
def get_new_key_file():
    ...
    new_key_path.place(x=165, y=280)
...
key_path = Entry(window, width=70).place(x=165, y=20)
...
created_key_path = Entry(window, width=70).place(x=165, y=280)
# Генерация ключа
create_new_key_now = Button(master=window, text='Генерация ключа', command=create_new_key_now).place(x=255, y=320)


info = Label(master=window, text='SenseFlare Lab').place(x=515, y=350)
